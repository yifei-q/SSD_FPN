...
def focal_loss(logits, labels, alpha=0.25, gamma=2):
    #pos_pt = logits
    pos_pt = tf.clip_by_value(tf.nn.softmax(logits),1e-10,0.999)
    labels = tf.one_hot(labels, depth=pos_pt.shape[1])
    #fl = labels*tf.log(pos_pt)*tf.pow(tf.ones_like(labels)-pos_pt,gamma)*alpha + (tf.ones_like(labels)-labels)*tf.log(tf.ones_like(labels)-pos_pt)*tf.pow(pos_pt,gamma)*(1-alpha)
    fl = labels * tf.log(pos_pt) * tf.pow(1 - pos_pt, gamma) * alpha #+ (1 - labels) * tf.log(1 - pos_pt) * tf.pow(pos_pt, gamma) * (1 - alpha)
    #fl = tf.reduce_mean(tf.reduce_sum(fl,axis=1))
    fl = -tf.reduce_mean(tf.reduce_sum(fl,axis=1))
    return fl
...
def ssd_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=tf.AUTO_REUSE):
        ...
        cls_pred = tf.reshape(cls_pred, [-1, params['num_classes']])#cls_pred = tf.reshape(cls_pred, [-1])
    ...
    with tf.device('/gpu:0'):
        with tf.control_dependencies([cls_pred, location_pred]):
            with tf.name_scope('post_forward'):
                ...
                #_cls_targets = tf.to_float(cls_targets[:, tf.newaxis] == [tf.range(params['num_classes'])])
                #print(_cls_targets)
                ...
                #selected_neg_mask = batch_negtive_mask
                ...
                #final_mask = tf.stop_gradient(tf.logical_or(tf.reshape(selected_neg_mask,[-1]), positive_mask))
    ...
    #cross_entropy = focal_loss(logits=cls_pred, labels=flaten_cls_targets)#*(params['negative_ratio'] + 1.)
    #cross_entropy = cross_entropy/tf.reduce_sum(tf.cast(positive_mask,tf.float32))
