...
class ReLuLayer(tf.layers.Layer):
    def __init__(self, name, **kwargs):
        super(ReLuLayer, self).__init__(name=name, trainable=True, **kwargs)
        self._name = name
...
def conv(name, inputs, nums_out, k_size, stride, padding, is_final=False):
    ...
    with tf.variable_scope(name):
        ...
        inputs = tf.nn.conv2d(inputs, W, [1, stride, stride, 1], padding,data_format="NCHW")
        inputs = tf.nn.bias_add(inputs, b)
    return inputs

def merge(name, lateral, top_down,filters=256):
    with tf.variable_scope(name):
        #lateral = conv("conv", lateral, filters, 1, 1, "SAME")
        lateral = tf.layers.conv2d(lateral,filters,kernel_size=(1,1),strides=(1,1),padding="same",data_format="channels_first",kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
        h, w = tf.shape(lateral)[2], tf.shape(lateral)[3]
        top_down = tf.transpose(top_down, perm=[0, 2, 3, 1])
        top_down = tf.image.resize_bilinear(top_down, [h, w])
        top_down = tf.transpose(top_down, perm=[0, 3, 1, 2])
...
class VGG16Backbone(object):
    ...
    def forward(self, inputs, training=True):
        ...
        for conv in self._conv3_block:
            inputs = forward_module(conv, inputs, training=training)
        """
        with tf.variable_scope('conv3_3_scale') as scope:
            weight_scale = tf.Variable([1.] * 256, trainable=training, name='weights')
        ...
        p1=feature_layers[5]
        p1=tf.layers.conv2d(inputs=p1,filters=256,kernel_size=1,strides=(1,1),padding="same",data_format=self._data_format,kernel_initializer=tf.glorot_uniform_initializer(),bias_initializer=tf.zeros_initializer())
        new_feature_layers.append(p1)
        p2 = merge("FPN2",feature_layers[4], p1)
        new_feature_layers.append(p2)
        p3 = merge("FPN3",feature_layers[3], p2)
        new_feature_layers.append(p3)
        #p3=tf.layers.conv2d(p3,512,1,(1,1),padding="same",data_format="channels_first")
        p4 = merge("FPN4",feature_layers[2], p3)
        new_feature_layers.append(p4)
        #p4 = tf.layers.conv2d(p4,1024,1,(1,1),padding="same",data_format="channels_first")
        p5 = merge("FPN5",feature_layers[1], p4)
        new_feature_layers.append(p5)
        #p5 = tf.layers.conv2d(p5,512,1,(1,1),padding="same",data_format="channels_first")
        p6 = merge("FPN6",feature_layers[0], p5)
        new_feature_layers.append(p6)
        new_feature_layers = new_feature_layers[::-1]
...
def loc_pred(features):
    features = tf.layers.conv2d(features, filters=256, kernel_size=3, strides=(1, 1), padding="same",data_format="channels_first", activation=tf.nn.relu,kernel_initializer=tf.glorot_uniform_initializer(),bias_initializer=tf.zeros_initializer())
    features = tf.layers.conv2d(features, filters=256, kernel_size=3, strides=(1, 1), padding="same",data_format="channels_first", activation=tf.nn.relu,kernel_initializer=tf.glorot_uniform_initializer(),bias_initializer=tf.zeros_initializer())
    #features = tf.layers.conv2d(features, filters=256, kernel_size=3, strides=(1, 1), padding="same",data_format="channels_first", activation=tf.nn.relu,kernel_initializer=tf.glorot_uniform_initializer(),bias_initializer=tf.zeros_initializer())
    return features

def cls_pred(features):
    features = tf.layers.conv2d(features, filters=256, kernel_size=3, strides=(1, 1), padding="same",data_format="channels_first", activation=tf.nn.relu,kernel_initializer=tf.glorot_uniform_initializer(),bias_initializer=tf.zeros_initializer())
    features = tf.layers.conv2d(features, filters=256, kernel_size=3, strides=(1, 1), padding="same",data_format="channels_first", activation=tf.nn.relu,kernel_initializer=tf.glorot_uniform_initializer(),bias_initializer=tf.zeros_initializer())
    #features = tf.layers.conv2d(features, filters=256, kernel_size=3, strides=(1, 1), padding="same",data_format="channels_first", activation=tf.nn.relu,kernel_initializer=tf.glorot_uniform_initializer(),bias_initializer=tf.zeros_initializer())
    return features
...
def multibox_head(feature_layers, num_classes, num_anchors_depth_per_layer, data_format='channels_first'):
    with tf.variable_scope('multibox_head'):
        ...
        for ind, feat in enumerate(feature_layers):
            """feat = tf.layers.conv2d(inputs=feat, filters=256, kernel_size=3, strides=(1, 1), padding="same",
